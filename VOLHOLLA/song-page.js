(function () {
  ...
  function normalizeAlbumInput(rawValue) {
    ...
    if (raw === "rootresidue" || raw === "root" || raw === "root-and-residue" || raw === "root_and_residue" || raw === "root and residue" || raw === "rar") {
      return "rootresidue";
    }
    if (raw === "nobrightstand" || raw === "no-bright-stand" || raw === "no_bright_stand" || raw === "no bright stand" || raw === "nbs") {
      return "nobrightstand";
    }
    ...
    if (raw === "doppler" || raw === "doppler-phantom" || raw === "doppler_phantom" || raw === "doppler phantom" || raw === "dp") {
      return "doppler";
    }
    if (raw === "aeolian" || raw === "aeolian-drift" || raw === "aeolian_drift" || raw === "aeolian drift" || raw === "ad") {
      return "aeolian";
    }
    ...
    if (raw === "inthemiddle" || raw === "middle" || raw === "in-the-middle" || raw === "in_the_middle" || raw === "in the middle" || raw === "itm") {
      return "inthemiddle";
    }
    if (raw === "mareamemory" || raw === "marea" || raw === "memory" || raw === "marea-memory" || raw === "marea_memory" || raw === "marea memory" || raw === "mm") {
      return "mareamemory";
    }
    ...
    if (raw === "brokemachine" || raw === "what-broke-the-machine" || raw === "what_broke_the_machine" || raw === "what broke the machine" || raw === "broken machine" || raw === "machine broke") {
      return "brokemachine";
    }
    if (raw === "coldrun" || raw === "down-in-the-cold-run" || raw === "down_in_the_cold_run" || raw === "down in the cold run" || raw === "cold run" || raw === "trout town") {
      return "coldrun";
    }
    if (raw === "siltwire" || raw === "silt-wire-and-mountain-current" || raw === "silt_wire_and_mountain_current" || raw === "silt wire and mountain current" || raw === "silt, wire & mountain current" || raw === "mountain current") {
      return "siltwire";
    }
    ...
    if (raw === "coatvectors" || raw === "coat" || raw === "vectors" || raw === "coat-of-many-vectors" || raw === "coat_of_many_vectors" || raw === "coat of many vectors" || raw === "coat-of-many-808s" || raw === "coat of many 808s" || raw === "808s" || raw === "comv") {
      return "coatvectors";
    }
    ...
    if (raw === "weedfield" || raw === "weed" || raw === "weed-choked-field" || raw === "weed_choked_field" || raw === "the weed-choked field" || raw === "the-weed-choked-field" || raw === "wcf") {
      return "weedfield";
    }
      ...
      : raw === "crossroads-and-crown" || raw === "crossroads_and_crown" || raw === "cac"
        ...
        : raw === "solar-reveries" || raw === "solar_reveries" || raw === "solar"
          ...
          : raw === "jukebox-time-collapse" || raw === "jukebox_time_collapse" || raw === "jtc"
            ? "jukebox"
            : raw === "neon-puddles" || raw === "neon_puddles" || raw === "neon"
              ? "neon"
